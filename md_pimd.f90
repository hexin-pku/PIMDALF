...
implicit none
    integer :: i, j
    do i, md_nbead
        call SES_dV(box_fx(:,i), box_x(:,:), md_nbead)
    enddo
...
implicit none
    ...
    !------ Here take SES model as example ----
    md_nfree = SES_rdim
    allocate(sys_m(SES_rdim))
    sys_m = SES_M
    !-- revised to harmonic oscillitor
    md_nfree = 1
    sys_m = 1
    !------------------------------------------
    ...
    allocate(md_ana_vals(md_nfree, 10))
...
implicit none
    ...
    !-- without header
    !write(trj_unit,*) 'nstep    ','nbead    ','ks   ','p    ','x    ','fks  ', 'fx   '
...
implicit none
    ...
    c1 = exp( - md_coeff * md_dtime/ real(n_dvd) )
    c2prime = sqrt( 1 - c1**2 )
        ...
        do i=1, md_nbead
            select case (md_thermo_flg)
                ...
                case (1)
                    ...
                    box_p(j,i) = c1 * box_p(j,i) + sqrt( box_m(j,i) * md_temp ) * c2prime * et
                !-- Anderson
                case (2)
...
implicit none
    ...
    do j=1, md_nfree
        do i=1, md_nbead
            box_ks(j,i) = box_ks(j,i) + box_p(j,i) * md_dtime / ( real(n_dvd) * box_m(j,i) )
        enddo
    enddo
    !-- relative updating
    do j=1, md_nfree
...
implicit none
    ...
    integer :: i, j
    do j=1, md_nfree
        box_p(j,1) = box_p(j,1) - box_fks(j,1) * md_dtime / real(n_dvd)
        do i=2, md_nbead
            box_p(j,i) = box_p(j,i) - box_fks(j,i) * md_dtime / real(n_dvd) - box_m(j,i) * md_bf2 * box_ks(j,i)
        enddo
    enddo
end subroutine md_update_p
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
