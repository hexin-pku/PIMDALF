...
implicit none
    ...
    real(8), parameter :: wn_2_hatree = 219474.6313702_8
    ...
    W(:)   = (/212.2_8, 209.0_8, 205.8_8, 202.7_8, 199.5_8, 196.3_8, 193.1_8/) / wn_2_hatree
...
implicit none
    ...
    dV = 0.0_8
    do i=1,vdim
        eterm = dexp(-dsqrt(0.5_8*M(1)*W(i)**2/D(i))*(R(1)-Req(i)))
        dV(1,i,i) = (1.0_8-eterm) * eterm * dsqrt(2.0_8 *M(1)*W(i)**2 * D(i))
        do j=i+1,vdim
            dV(1,i,j) = - C(i,j) * dexp( - a1*(R(1)-Rc(i,j))**2 ) * &
               ( ( 2.0_8*a1*(R(1)-Rc(i,j)) )* dcos(a2*(R(1)-Rc(i,j))) & 
