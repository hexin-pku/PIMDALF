...
private
    ...
    real(8), dimension(:,:,:), allocatable   :: save_EVds, save_EVnds, save_Os, save_OOs, save_Ts, save_Ls
...
subroutine init_mes()
    ...
    allocate(save_Ls(nbead,vdim,vdim))
    allocate(save_Ts(nbead,vdim,vdim))
...
implicit none
    real(8), dimension(ndof, nbead), intent(in) :: Rs
    real(8), dimension(vdim,vdim) :: PFD_matrix, V, Vd, Vnd, O, OO, Y, T, L
    real(8), intent(out) :: esval
    real(8), dimension(vdim) :: Es
    integer :: i
    ...
    do i=1, size(Rs,dim=2)
        ...
        !-- we use EVds, EVnds to save eigenvalues and eigenvectors
        !-- save XXX
        save_Ls(i,:,:) = L
        save_Ts(i,:,:) = T
        
        O = matmul( T, matmul( lin_expdiag(-0.5*betap*L), transpose(T) ) )
...
subroutine getOpR(OpR, R, frame, iflag)             
    ...
    elseif(flag .eq. 2) then
        do j=1,size(R)
            L = save_Ls(frame,:,:)
            T = save_Ts(frame,:,:)
            dV = save_dVds(j,frame,:,:) + save_dVnds(j,frame,:,:)
            OpR(j,:,:) = OpR_diag(dV, L, T, 0.5_8*betap )
        enddo
    else
        stop "flag error"
    endif
...
recursive function OpR_diag(dV, L, T, a) result(M)
    real(8), dimension(vdim,vdim), intent(in) :: dV, L, T
    real(8), intent(in) :: a
    real(8), dimension(vdim,vdim) :: Ev, term, TdVT
    real(8), dimension(vdim,vdim) :: M, M_old
    integer :: i,j,factor
    
    if(a > 0.5) then
        
        Ev = matmul(T, matmul( lin_expdiag(-0.5_8*a*L), transpose(T) ) )
        term = OpR_diag(dV, L, T, 0.5*a)
        M = matmul( term,  Ev) + matmul(Ev, term )
    else
        
        M = 0.0
        factor = 1
        TdVT = matmul( transpose(T), matmul( dV, T))
        
        do i=1,20
            factor = factor * i ! factor = i!       
            term = 0.0
            do j=0,i-1
                term = term + matmul( lin_powerdiag(L,j), matmul( TdVT, lin_powerdiag(L,i-1-j) ) )
            enddo
            M = M + (-a)**i / real(factor) * term
            
            if(maxval(abs( (-a)**i / real(factor) * term )) < 1e-20) then
                exit
            endif

        enddo
        M = matmul( T, matmul( M, transpose(T) ) )       
    endif
end function OpR_diag
